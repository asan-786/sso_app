...
export class BaseAjax {
    constructor() {
        ...
        this.caches = new Map(); // 缓存所有已经请求的Promise，同一时间重复的不再请求
        this.IS_AJAX_STOP = false;
    }
    /**
     * 停止ajax
     */
    ...
    handleGetUrl(url, data, isEncodeUrl) {
        let tempUrl = url;
    ...
    async request(config) {
        const { url, baseURL, //前缀url
        data, query, headers = {}, method, credentials, isFile, isUseOrigin, isEncodeUrl, //get请求时是否要进行浏览器编码
        ignore, defaultPutAndPostContentType, ...otherParams } = config;
    ...
    mergeAbortConfig(config, signal) {
        let controller;
    ...
    mergeConfig(cfg) {
        ...
        for (let i = 0; i < chain.length; i += 2) {
            try {
                ...
                console.error(e);
                (_a = chain[i + 1]) === null || _a === void 0 ? void 0 : _a.call(chain, e); // TODO 这个作用没想好
                break;
            }
        }
        return config;
    }
    mergeResponse(promise) {
    ...
    clearCacheByKey(uniqueKey, cacheTimeout) {
     ...
     * @param config
     **/
    fetch_timeout(fecthPromise, controller, config) {
        let tp;
     ...
     */
    getAbortResult(url, data, options) {
        return this.ajaxAbortResult({
            url,
            method: "get",
            data,
            ...options,
     ...
     */
    postAbortResult(url, data, options) {
        return this.ajaxAbortResult({
            url,
            method: "post",
            data,
